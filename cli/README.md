...
A zero-dependency command-line interface to the BaseData desktop app, built for
...
```bash
node cli/basedata.mjs help
```

Or link it as a global `basedata` command:
...
> You can manage my BaseData projects with `node /path/to/basedata/cli/basedata.mjs`.
...
(`~/Library/Application Support/basedata` on macOS). Override with the
`BASEDATA_DIR` environment variable or `--data-dir` — handy for testing
